...
- **规则库管理**：上传 Markdown 或 JSON 格式规则文件，按规则库分类管理（CHUNK / SAR 各一套）；SAR 侧支持 Excel 检查单导入（`/sar/rules/import-checklist`）拆解为原子检查项
- **规则上传冲突处理**：同一规则库/文件夹下上传同名来源文件时，系统先提示冲突项；用户可选择保留现有规则，或确认替换后删除旧规则再导入新规则，避免重复追加
...
│   │   │   ├── RuleController / SarRuleController # 规则；SAR 侧含 /import-checklist，CHUNK / SAR 均支持上传冲突检查
...
| CHUNK 场景/规则/规则库 | /api/v1/scenarios·rules·rule-libraries/** | 增删改查、规则上传、上传冲突检查 `/rules/upload-conflicts` |
| SAR 审查 | /api/v1/sar/reviews/** | 与 CHUNK 审查同构（结构化精准管线） |
| SAR 场景/规则/规则库 | /api/v1/sar/scenarios·rules·rule-libraries/** | SAR 侧；规则上传、上传冲突检查 `/sar/rules/upload-conflicts`、Excel 检查单导入 `/sar/rules/import-checklist` |
...
规则上传接口支持 `replaceExisting=false|true`。默认 `false` 时，如果同一规则库/文件夹内存在相同来源文件，后端返回冲突错误，前端会展示冲突规则名称、编码、来源文件和更新时间；用户确认替换后以 `replaceExisting=true` 重新上传，后端先删除冲突规则，再导入新规则。SAR 检查单导入同样支持该参数，冲突判断使用系统生成的规则 JSON 文件名。
