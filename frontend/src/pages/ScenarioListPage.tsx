...
function ScenarioListPage() {
  ...
  const columns: ColumnsType<Scenario> = [
    ...
    },
    {
      ...
      width: 120,
      render: (_, record) => (
        <Tag color="blue">{record.ruleIds?.length ?? 0} 条</Tag>
      ),
    },
    {
      title: '创建时间',
  ...
  return (
    <div>
          ...
          >
            <Checkbox.Group style={{ width: '100%' }}>
              <div style={{ maxHeight: 300, overflow: 'auto' }}>
                ...
                {allRules.length === 0 && (
                  <Text type="secondary">暂无可用规则，请先上传规则</Text>
                )}
              </div>
            </Checkbox.Group>
          </Form.Item>
      ...
      {/* Detail Modal */}
      <Modal
        title={`场景详情 - ${detailScenario?.name || ''}`}
        ...
        {detailScenario && (
          <div>
            ...
            <List
              size="small"
              bordered
              dataSource={detailScenario.rules || []}
              renderItem={(rule) => (
                <List.Item>
                  <Space>
